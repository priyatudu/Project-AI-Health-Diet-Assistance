...
# st.page_setup(page_title ="Health Assistance",
#               page_icon="🤖",
#               layout ="wide")

st.set_page_config(layout = "wide")
st.title("AI Health & Diet Assistance 🤖") #if pagesetup is not working


st.write("Personal Health Asssistance and Diet Recommendation Agent")
st.header("Health Information")


st.sidebar.header("Your Imformatiom🎫")
...
activity = st.sidebar.selectbox("Activity",["Sedentary","Lightly_Active","Moderately_active","Very_active","Extra_active"])
aim = st.sidebar.selectbox("AIM",["Weight_maintain","Weight_loss","Weight_gain"])
diet_type = st.sidebar.selectbox("Diet Type",["Vegetarian","Non-vegiterian"])
allergies = st.sidebar.selectbox("Allergies",["Allergy","None"])
...
calories = Calories_Target(tdee,aim)
...
tab1,tab2 = st.tabs(['Diet Recommendation','Health Assistence'])
if tab1:
    if st.button(label="Recommend Diet"):
        if client:
            with st.spinner("Creating Diet"):
                try:
                    db = load_rag()
                    search_query = f""" diet_type {diet_type} Healthy food Protein Allergies {allergies}"""
...
# """
    ...
    question = st.text_area("Ask about Health",
                 placeholder="eg:Good Source of vegeterian protin")
    if st.button("Ask Ai"):
        db = load_rag()
